...
class UserBowl extends StatefulWidget {
  const UserBowl(this.changeScreen, this.target, this.innings, {super.key});
  final void Function(int data, int target) changeScreen;
  final int target, innings;
  @override
  State<UserBowl> createState() {
    return _UserBowlState();
  }
}

class _UserBowlState extends State<UserBowl> {
  ...
  void score(int data) {
    setState(() {
      ...
      if (data == temp) {
        if (widget.innings == 0) {
          widget.changeScreen(total + 1, 1);
        } else {
          widget.changeScreen(total, 3);
        }
  ...
  Widget build(context) {
    ...
    if (widget.target == 9999) {
      ...
      tar = 'The target is ${widget.target}';
    }
    return Center(
      child: Column(
